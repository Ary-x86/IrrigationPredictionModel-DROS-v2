...
def calculate_capacity_and_merge():
    ...
    # Replace the "Generating AI target classes..." section with this:

    print("Generating AI target classes using the paper's row-wise policy logic...")

    # Default = No Adjustment (2)
    df["Irrigation_Decision"] = 2

    moisture = df["Soil Moisture [RH%]"]
    rainfall = df["Weather Forecast Rainfall [mm]"]

    dry_mask = moisture < lower_limit_standard
    wet_mask = moisture > upper_limit_standard
    rain_mask = rainfall > 2.0
    critical_mask = moisture <= critical_limit  # use <= so the boundary falls into ALERT

    # 1) Above the upper confidence limit -> OFF
    df.loc[wet_mask, "Irrigation_Decision"] = 0

    # 2) Below the lower confidence limit and no significant rain -> ON
    df.loc[dry_mask & (~rain_mask), "Irrigation_Decision"] = 1

    # 3) Below the lower confidence limit, rain is significant, but NOT critically dry -> OFF
    # This is the branch from Fig. 3 that was previously misread.
    df.loc[dry_mask & rain_mask & (~critical_mask), "Irrigation_Decision"] = 0

    # 4) Below the lower confidence limit, rain is significant, and critically dry -> ALERT
    df.loc[dry_mask & rain_mask & critical_mask, "Irrigation_Decision"] = 3
    ...
    metadata = {
        ...
        "labeling_mode": "row_wise_policy_logic",
    }
